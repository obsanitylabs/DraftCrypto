    ...
    <div className="px-4 pb-10 max-w-3xl mx-auto">
        ...
        <div className="flex gap-1">
          {(['h2h', 'leagues'] as const).map(t => (
            <button
              ...
              onClick={() => setTab(t)}
              className={cn(
                'text-xs px-3 py-1 font-mono border transition-all rounded-md',
      ...
      {tab === 'h2h' ? (
        ...
        <Card className="p-5 text-center">
          <div className="text-sm text-fc-text-dim">
      ...
      <Modal open={settingsModal} onClose={() => setSettingsModal(false)} title="Edit Profile">
        <div className="space-y-4 mb-5">
          <div>
            <label className="text-xs text-fc-text-dim tracking-wider block mb-1.5">X (TWITTER) HANDLE</label>
            <Input
              value={xHandle}
          ...
          <div>
            <label className="text-xs text-fc-text-dim tracking-wider block mb-1.5">TELEGRAM HANDLE</label>
            <Input
              value={tgHandle}
          ...
          <div>
            <label className="text-xs text-fc-text-dim tracking-wider block mb-1.5">DEFAULT TRADE MODE</label>
            <div className="flex gap-2">
              {(['paper', 'live'] as TradeMode[]).map(mode => (
                <button
                  key={mode}
                  className={cn(
                    'flex-1 py-2.5 text-xs font-mono border transition-all rounded-md',
        ...
        <div className="mt-5 pt-4 border-t border-fc-border">
...
function StatCard({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div className="border border-fc-border bg-fc-card p-3 text-center rounded-md">
      <div className={cn('text-base font-bold', color)}>{value}</div>
      <div className="text-xs text-fc-text-dim">{label}</div>
    </div>
  );
}
    ...
    <Card hover className="p-4">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-3">
          <div className={cn(
            'w-7 h-7 flex items-center justify-center text-xs font-bold border rounded-md',
            match.won
              ? 'border-fc-border-green bg-fc-green-glow text-fc-green'
          ...
          <div>
            <div className="text-sm font-bold">vs {match.opponent}</div>
