    ...
    <PageShell>
      <div className="px-4 py-8 pb-16 space-y-10 max-w-3xl mx-auto">
        ...
        <Step number={0} title="THE CONCEPT" accent>
          <p>DraftCrypto is like fantasy football, but for crypto markets. You draft a portfolio of tokens, set each as Long or Short, and compete against an opponent. Whoever has the better PnL at the end of the match wins the pot.</p>
        </Step>
        ...
        <Step number={2} title="THE DRAFT">
          ...
          <div className="fc-card p-4 mt-4 space-y-3">
          ...
          <p className="mt-4">Each pick has a 30-second timer. If time runs out, auto-pick selects the highest market cap token remaining.</p>
          <p className="mt-3">Snake order means if you pick first in Round 1, your opponent picks first in Round 2. This keeps it fair.</p>
        </Step>
        ...
        <Step number={3} title="SET YOUR LINEUP">
          ...
          <p className="mt-4">Then set mandatory Take-Profit (TP) and Stop-Loss (SL) for each active position. These auto-close your position when triggered.</p>
        </Step>
        ...
        <Step number={4} title="USE BOOSTS">
          ...
          <div className="fc-card p-4 mt-4 space-y-2">
          ...
          <p className="mt-4">A Round 1 pick (25% weight) with a 3x boost becomes 75% of your total PnL. High risk, high reward.</p>
        </Step>
        ...
        <Step number={6} title="SETTLEMENT">
          ...
          <div className="fc-card p-4 mt-4 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-sm text-fc-text-dim tracking-wider">Winner receives</span>
              <span className="text-sm text-fc-green font-mono tracking-wider">95% of total pot</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-fc-text-dim tracking-wider">Platform fee</span>
              <span className="text-sm text-fc-text-muted font-mono tracking-wider">5%</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-fc-text-dim tracking-wider">UNITE earned (winner)</span>
              <span className="text-sm text-fc-green font-mono tracking-wider">100 UNITE</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-fc-text-dim tracking-wider">UNITE earned (loser)</span>
              <span className="text-sm text-fc-text-muted font-mono tracking-wider">10 UNITE</span>
            </div>
          </div>
        </Step>
        ...
        <Step number={7} title="LEAGUES (12-WEEK SEASON)">
          ...
          <p className="mt-3">You can invite a co-manager to help manage lineups, or rage quit if things go south (but you&apos;ll forfeit all remaining matches).</p>
        </Step>

        {/* CTA */}
        <div className="fc-card fc-accent-top p-8 text-center space-y-4">
          <p className="text-base font-mono tracking-widest font-bold text-fc-green">READY TO DRAFT?</p>
          <p className="text-sm text-fc-text-muted tracking-wider">
            Start with Paper mode for free. No wager required.
          </p>
          <div className="flex gap-3 justify-center mt-3">
            <Link
              href="/lobby"
              className="fc-btn-primary px-5 py-3 text-sm font-mono tracking-widest"
            >
              FIND A MATCH
            </Link>
            <Link
              href="/faq"
              className="fc-btn-secondary px-5 py-3 text-sm font-mono tracking-widest"
            >
              READ FAQ
            </Link>
          </div>
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-3">
        {number > 0 && (
          <span className="w-9 h-9 flex items-center justify-center bg-fc-green/10 text-fc-green text-sm font-mono font-bold shrink-0 rounded-md">
            {number}
          </span>
        )}
        <h2 className={`text-sm font-mono tracking-widest font-semibold ${accent ? 'text-fc-green' : 'text-fc-text'}`}>
          {title}
        </h2>
      </div>
      <div className="text-sm text-fc-text-muted tracking-wide leading-relaxed space-y-2 pl-12">
        {children}
      </div>
    </div>
  );
...
function Card({ label, desc }: { label: string; desc: string }) {
  return (
    <div className="fc-card p-4">
      <div className="text-xs font-mono tracking-widest text-fc-text font-semibold">{label}</div>
      <div className="text-sm text-fc-text-dim tracking-wider mt-1.5">{desc}</div>
    </div>
  );
}
...
function DraftRow({ round, label, weight, desc }: { round: number; label: string; weight: string; desc: string }) {
  return (
    <div className="flex items-center gap-4">
      <span className="text-sm font-mono text-fc-green w-10 shrink-0">{weight}</span>
      <div>
        <span className="text-sm font-mono tracking-wider text-fc-text">{label}</span>
        <span className="text-sm text-fc-text-dim tracking-wider ml-2">— {desc}</span>
      </div>
    </div>
  );
}
...
function PositionCard({ type, color, emoji, desc }: { type: string; color: string; emoji: string; desc: string }) {
  return (
    <div className="flex-1 fc-card p-4 text-center">
      <div className="text-2xl">{emoji}</div>
      <div className={`text-xs font-mono tracking-widest font-semibold mt-2 ${color}`}>{type}</div>
      <div className="text-sm text-fc-text-dim tracking-wider mt-1">{desc}</div>
    </div>
  );
}
...
function TierRow({ tier, unite, boosts, multiplier }: { tier: string; unite: string; boosts: string; multiplier: string }) {
  return (
    <div className="flex items-center justify-between text-sm font-mono tracking-wider">
